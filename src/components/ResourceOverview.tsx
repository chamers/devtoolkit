...
export default function ResourceOverview(props: Props) {
  ...
  // 💡 NEW: Safely get the first logo URL (the header image)
  const logoUrl = logoUrls.length > 0 ? logoUrls[0] : null;
    ...
    >
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12 xl:gap-16">
            <div className="flex-1 space-y-6 lg:space-y-7">
              <div className="space-y-4 lg:space-y-5">
                ...
                {/* 💡 CHANGE: Wrap the logo and title in a flex container */}
                <div className="flex items-center gap-4">
                  {/* Render the Logo (index 0 image) */}
                  {logoUrl && (
      ...
      {logoUrls.length > 0 && (
        <div className="hidden md:flex md:flex-col md:flex-1 md:justify-start relative rounded-2xl shadow-2xl dark:shadow-[-20px_0_30px_-10px_rgba(255,165,0,0.4)] border border-neutral-200 dark:border-neutral-800 p-4 lg:p-5 xl:p-6">
          <SliderWrapper
            // 💡 IMPORTANT: These arrays should now be sliced in SliderWrapper.tsx
            // if you want to reserve index 0 for the logo/header.
            images={logoUrls}
